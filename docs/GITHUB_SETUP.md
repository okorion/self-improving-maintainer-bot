...
로컬 Codex 전용 운영에서는 `OPENAI_API_KEY`를 설정하지 않습니다. GitHub에는 자동 PR 생성을 위한 token만 설정합니다.
...
로컬 Codex 전용 운영에서는 필수 repository variable이 없습니다.

대상 레포 정보를 GitHub Actions에도 노출하고 싶다면 다음 variable을 추가할 수 있습니다.
...
- PR과 수동 실행에서 local dry-run eval 실행
- API secret을 사용하지 않음
...
- 로컬에서 `python -m self_maintainer_bot.cli codex-status`가 통과하는지
- `TARGET_REPOSITORY`가 private이면 `gh auth status`와 `git ls-remote`가 통과하는지
- `CODEX_TIMEOUT_SECONDS`가 너무 짧지 않은지
