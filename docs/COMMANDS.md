...
```bash
python -m maintainer_bot.cli add-eval \
  --id install-002 \
...
```bash
python -m maintainer_bot.cli triage-issue \
...
```bash
GITHUB_TOKEN=... python -m maintainer_bot.cli sync-labels --repo OWNER/REPO
```
...
```bash
GITHUB_TOKEN=... python -m maintainer_bot.cli apply-issue-labels \
  --repo OWNER/REPO \
  --issue-number 1 \
  --title "Docs typo in README" \
  --body "The installation guide has a typo"
...
```bash
GITHUB_TOKEN=... python -m maintainer_bot.cli comment-pr-summary \
  --repo OWNER/REPO \
  --pr-number 12 \
